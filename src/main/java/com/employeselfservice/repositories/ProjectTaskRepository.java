...
public interface ProjectTaskRepository extends JpaRepository<ProjectTask,Long> {
    ...
    // Updated method to find all tasks assigned to an employee by project member with status DONE
    @Query("SELECT pt FROM ProjectTask pt " +
            "JOIN pt.projectMember pm " +
            "WHERE pm.employee.id = :employeeId " +
            "AND pt.status = 'DONE'")
    List<ProjectTask> findAllTasksByEmployeeIdAndStatusDone(@Param("employeeId") Long employeeId);
